...
.DESCRIPTION
    ...
    shortcut so you can launch the GUI without touching the command line:
      ...
      5. Shortcuts that launch the audio_fingerprint.gui package with the venv's Python
...
Write-Host "======================================================================"
Write-Host "  EpisodeSleuth - Windows setup"
Write-Host "======================================================================"
...
if ($NoVenv) {
    ...
    # Install the project itself (editable) so the audio_fingerprint.gui package
    # and the dvd-gui / dvd-identify console scripts resolve from any directory.
    & $python -m pip install -e $ProjectDir | Out-Host
    ...
    # Install the project itself (editable) so the audio_fingerprint.gui package
    # and the dvd-gui / dvd-identify console scripts resolve from any directory.
    & $venvPy -m pip install -e $ProjectDir | Out-Host
}
...
$launcher = Join-Path $ProjectDir "Launch_EpisodeSleuth.bat"
...
cd /d "%~dp0"
start "" "$pywForBat" -m audio_fingerprint.gui
"@ | Set-Content -Encoding ASCII $launcher
Ok "Wrote launcher: Launch_EpisodeSleuth.bat"
...
if (-not $NoShortcut) {
    ...
    function New-Shortcut($linkPath) {
        ...
        if (Test-Path $venvPyw) {
            ...
            $sc.Arguments  = '-m audio_fingerprint.gui'
        } else {
        ...
        $sc.Description = "EpisodeSleuth - identify DVD-ripped episodes for Plex"
        $sc.Save()
    }

    $desktop = [Environment]::GetFolderPath("Desktop")
    New-Shortcut (Join-Path $desktop "EpisodeSleuth.lnk")
    Ok "Created Desktop shortcut."

    $startMenu = Join-Path ([Environment]::GetFolderPath("Programs")) "EpisodeSleuth"
    New-Item -ItemType Directory -Force -Path $startMenu | Out-Null
    New-Shortcut (Join-Path $startMenu "EpisodeSleuth.lnk")
...
Write-Host "  - double-clicking the 'EpisodeSleuth' Desktop shortcut, or"
Write-Host "  - double-clicking Launch_EpisodeSleuth.bat, or"
Write-Host "  - running:  $pyForBat -m audio_fingerprint.gui"
Write-Host ""
