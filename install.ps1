...
.DESCRIPTION
    ...
    shortcut so you can launch the GUI without touching the command line:
      ...
      6. Shortcuts that launch dvd_identifier_gui.py with the venv's Python
...
cd /d "%~dp0"
start "" "$pywForBat" "dvd_identifier_gui.py"
...
if (-not $NoShortcut) {
    ...
    function New-Shortcut($linkPath) {
        ...
        if (Test-Path $venvPyw) {
            ...
            $sc.Arguments  = '"' + (Join-Path $ProjectDir "dvd_identifier_gui.py") + '"'
        } else {
...
Write-Host "  - running:  $pyForBat dvd_identifier_gui.py"
Write-Host ""
